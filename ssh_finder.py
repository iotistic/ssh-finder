...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="SSH Finder script with parallel attempts, host filtering via ping, extra SSH options, "
...
""",
    ...
    host_group.add_argument(
        "--hosts-file", help="File containing list of hosts/subnets (default: hosts.txt)"
    )

    # Password options
    parser.add_argument(
        "-p", "--passwords", help="Comma-separated list of passwords (e.g., pass123,rootpass)"
    )
    parser.add_argument(
        "--passwords-file", help="File containing passwords (default: passwords.txt)"
    )

    # User options
    parser.add_argument(
        "-u", "--users", help="Comma-separated list of usernames (e.g., admin,root)"
    )
    parser.add_argument("--users-file", help="File containing multiple usernames (one per line)")

    # Logging and SSH options
    parser.add_argument(
        "-l",
        "--log-file",
        default="ssh_attempts.log",
        help="Log file location (default: ssh_attempts.log)",
    )
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Suppress most output (only warnings and errors)"
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Show detailed logs")
    parser.add_argument(
        "--ssh-options", help="Extra SSH options (e.g., '-p 2222 -o ConnectTimeout=5')", default=""
    )
    ...
    parser.add_argument(
        "-c",
        "--connect-on-first-success",
        action="store_true",
        help="Stop script and open interactive SSH session on first successful login.",
    )

    # Ping check options
    parser.add_argument("--skip-ping", action="store_true", help="Skip the ping check for hosts.")
    parser.add_argument(
        "--ping-timeout", type=int, default=1, help="Ping timeout in seconds (default: 1)."
    )
    parser.add_argument(
        "--ping-pool-size",
        type=int,
        default=MAX_PING_PROCESSES,
        help=f"Maximum number of ping processes (default: {MAX_PING_PROCESSES}).",
    )

    # Port check options
    parser.add_argument(
        "--skip-port-check", action="store_true", help="Skip checking if the SSH port is open."
    )
    parser.add_argument("--port", type=int, default=22, help="Port to check for SSH (default: 22).")
    parser.add_argument(
        "--port-timeout",
        type=int,
        default=1,
    ...
    parser.add_argument(
        "-s", "--secret", action="store_true", help="Enable secret mode for password input."
    )
...
def ping_host(host, ping_timeout):
    ...
    logging.debug(f"Checking ping for {host} with timeout {ping_timeout} sec...")
...
def check_reachable_hosts(hosts, args):
    ...
    if len(hosts) > 5:
        ...
        logging.debug(f"Using multiprocessing to ping {len(hosts)} hosts in parallel...")
        pool_size = min(args.ping_pool_size, len(hosts))
        with multiprocessing.Pool(pool_size) as pool:
            reachable_hosts = pool.starmap(ping_host, [(host, ping_timeout) for host in hosts])
        reachable = {host for host in reachable_hosts if host is not None}
    else:
        # For fewer hosts, we can ping sequentially
        reachable = {host for host in hosts if ping_host(host, ping_timeout) is not None}
...
def parse_hosts(args):
    ...
    """
    logging.info("Parsing hosts and filtering reachable hosts with open SSH port...")
    hosts = []
    lines = []
    ...
    if not args.skip_ping:
        logging.info(f"Checking ping for reachable hosts with timeout {args.ping_timeout} sec...")
    ...
    filtered_hosts = [host for host in reachable if is_ssh_port_open(host, args)]
...
def attempt_ssh_login(host, user, password, args):
    """
    Attempt an SSH login to the given host using the provided user and password.
    If successful, return True. Uses paramiko instead of sshpass.
    """
    logging.debug(
    ...
    try:
        ...
        # Automatically add host keys (similar to StrictHostKeyChecking=no)
        ...
        logging.debug(
            f"Authentication failed for {user}@{host} with password: {obfuscate_if_secret(password, args)}"
        )
        return False
    except Exception as e:
        logging.error(f"Error connecting to {host} as {user}: {str(e)}")
        return False
...
def generate_report(successful_combos, total_combinations, args):
    ...
    success_rate = (successful_count / total_combinations) * 100 if total_combinations > 0 else 0
    ...
    if successful_combos:
        ...
        sorted_combos = sorted(successful_combos, key=lambda x: x[0])  # Sort by host
        for host, user, password in sorted_combos:
            port_option = f"-p {args.port} " if args.port != 22 else ""
            report_lines.append(
                f"  ✅ Host: {host} | User: {user} | Password: {obfuscate_if_secret(password, args)}"
                f"      → SSH Command: ssh {port_option}{user}@{host}"
            )
    else:
...
def main():
    ...
    # Warn about ignored SSH options
    if args.ssh_options:
        logging.warning("SSH options are currently ignored when using paramiko")
    ...
    combinations = [
        (host, user, password) for user in users for password in passwords for host in hosts
    ]
    total_combinations = len(combinations)
    logging.debug(f"Attempting {total_combinations} connection combinations in parallel...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=pool_size) as executor:
        future_to_combo = {
            executor.submit(attempt_ssh_login, host, user, password, args): (host, user, password)
            for host, user, password in combinations
        }

        progress_bar = tqdm(
            total=total_combinations, desc="Trying combinations", disable=not use_progress_bar
        )
        ...
        for future in concurrent.futures.as_completed(future_to_combo):
            ...
            try:
                if future.result():
                    logging.debug(f"Login successful for {combo[1]}@{combo[0]}")
                    successful_combos.append(combo)
                    if args.connect_on_first_success:
                        progress_bar.close()
                        host, user, password = combo
                        port_option = f"-p {args.port} " if args.port != 22 else ""
                        print(
                            f"\nSUCCESS! To connect manually, run:\n  ssh {port_option}{user}@{host}"
                        )
                        sys.exit(0)
            except Exception as exc:
                logging.error(f"Exception during SSH attempt for {combo[1]}@{combo[0]}: {exc}")
